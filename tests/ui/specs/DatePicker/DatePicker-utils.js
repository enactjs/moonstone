...
async function expectNoLabels (picker) {
	expect(await picker.monthLabel.isExisting()).toBe(false);
	expect(await picker.dayLabel.isExisting()).toBe(false);
	expect(await picker.yearLabel.isExisting()).toBe(false);
}
